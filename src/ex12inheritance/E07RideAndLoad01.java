...
오버라이딩 : 메서드명, 매개변수, 반환형까지 몸두 동일한 메서드를
...
public class E07RideAndLoad01
{
	...
	public static void main(String[] args)
	{
		...
		따라서 접근범위는 C객체 전체가 되므로 아래외 호출문장은
		모두 실행된다.
		 */
		...
		ref2.loadMethod();
		ref2.loadMethod(1);
		//ref2.loadMethod(1.1);//접근불가 -> C클래스 영역은 접근불가
		
		// ref1는 A타입이므로 접근범위가 A까지로 제한된다.
		ref1.loadMethod();
		//ref1.loadMethod(1);//접근불가 -> B클래스 영역은 접근불가
		//ref1.loadMethod(1.1);//접근불가 -> C클래스 영역은 접근불가
		...
		자식으로 부모를 참조하는 것은 불가능 하므로 아래 2개의 문장은 에러가 
		발생한다.
		 */
		A refId1 = new C(); //가능
		...
		/*
		위에서 에러가 난 부분은 아래처럼 다운캐스팅(상제형변환) 하면
