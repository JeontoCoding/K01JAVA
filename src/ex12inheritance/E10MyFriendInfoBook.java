...
class HighFriend extends Friend {
	...
	super를 통해 호툴하고, 자식에서 확장한 변수를 별도의 print문을
	통해 출력한다.
	 */
	public void showAllData() {
...
class FriendIndoHandler {
	...
	public void addFriend(int choice) {
		...
		3.numOfFriend를 1 증가시킨다. 후위증가이므로 이렇게 처리된다.
		 */
		if(choice == 1) {
	...
	1.친구정보를 추가할 때 high또는 Univ 객체가 배열에 저장된다.
	2. 입력 시 모든 객체는 Friend로 자동 형변환되어 저장된다.
	3. 정보 출력 시 배열에 입력된 객체수 만큼 반복하면 각 요소를 통해
		멤버 메서드를 호출한다.
	...
	public void deleteInfo() {
		...
		/*
		배열의 요소 중 산제된 요소의 인덱스값을 저장할 용도의 변수.
		요소를 삭제한 후 빈자를 채워넣을 때 사용할 것임.
